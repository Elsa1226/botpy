...
MULTI_ROUND_DEFAULT_TEMPLATES = {
    "start": "【{task_name}】已启动！请发送任务内指令，或发送「结束任务」退出～",
    "end": "【{task_name}】已结束！",
    "timeout": "⚠️ 【{task_name}】会话超时，已自动结束～",
    "invalid_cmd": "❌ 当前仅支持【{task_name}】的指令：{valid_cmds}，或发送「结束任务」退出～"
}
...
MULTI_ROUND_TASK_TEMPLATES = {
    # 猜拳任务
    CommandType.GAME_ROCK_PAPER_SCISSORS.value: {
        "start": "✊ 猜拳游戏开始！可发送：石头/剪刀/布，或「结束任务」退出～",
        "end": "🎮 猜拳游戏结束！共玩{round_num}轮，你赢{user_win}轮～",  # 支持动态参数
        "valid_cmds": "石头、剪刀、布"
    },
    # # 猜谜语任务（未来扩展）
